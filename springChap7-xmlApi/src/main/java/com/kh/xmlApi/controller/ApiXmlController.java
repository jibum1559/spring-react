...
public class ApiXmlController {
	...
	public ResponseEntity<?> xmlApi() {
		//파일을 가지고올 때 항상 String Builder
		//파일 내용이 들어갈 Builder를 미리 세팅해서 
		//비어있는 Builder 안에 가져온 내용을 채울 예정
		StringBuilder result = new StringBuilder();
		
		//1.apiUrl 2.key 3.xml 형식으로 가지고 올 것
		//만약에 xml 파일이 url 주소값으로 가지고 오는 것이 아니라 파일 자체를 다운받아서 가지고 있다면 
		//1.apiUrl 2.Key 값을 넣어줄 의무가 없다
		//파일의 위치와 파일을 변환해주는 작업을 해주면 됨
		//파일을 가지고 올 때는 Buffer와 Stream을 사용한다.
		//최종적으로 json 형식으로 xml 파일이 변환해서 출력을 할 것
		
		//json 형식으로 가져올 때 key balue
		//추후에 return 데이터 목록
		List<Map<String, Object>> mapList = 
		//위 mapList에서 받아오는 형식과 = 을 써서 가져오는 형식이 같아야 한다.
new ArrayList<Map<String, Object>>();
		
		//return data key 목록을 담을 공간을 만들어주기
		...
		try {
			...
			while((line = br.readLine()) != null) {
				...
				if(headerList.size() == 0) {
					...
					//담아주는 값과 객체를 만들어주는 값을 동일하기 설정
					
					//읽어온 header 컬럼 갯수를 기준을 데이터 출력
